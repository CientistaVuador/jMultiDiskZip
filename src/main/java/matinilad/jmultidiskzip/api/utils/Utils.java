...
public class Utils {
    ...
    public static Path validateOutputFileExtension(Path outputFile, boolean encrypt, CompressionAlgorithm compression, OutputFormat outputFormat) {
        ...
        if (parent == null) {
            throw new IllegalArgumentException("output file has no parent: "+outputFile.toString());
        }
        Path nameFile = outputFile.getFileName();
        if (nameFile == null || nameFile.toString().isEmpty()) {
            throw new IllegalArgumentException("output file has no name: "+outputFile.toString());
        }
        ...
        if (PartOutputStream.getPartNumber(outputFile) != 1) {
            ...
            if (outputFormat != null) {
                filename += "." + outputFormat.getExtension();
            }
