...
public class UIUtils {
    ...
    public static Path validatePartFileExtension(Path outputFile, boolean encrypt, CompressionAlgorithm compression, OutputFormat outputFormat) {
        int partNumber = PartOutputStream.getPartNumber(outputFile);

        if (partNumber == -1) {
            String extensions = "";
            if (!encrypt) {
                extensions += "." + ZipCreator.EXTENSION;
                if (compression != null) {
                    extensions += "." + compression.getExtension(0);
                }
            } else {
                extensions += "." + EncryptedOutputStream.EXTENSION;
            }
            if (outputFormat != null) {
                extensions += "." + outputFormat.getExtension();
            }
            ...
            if (!filename.toLowerCase().endsWith(extensions.toLowerCase())) {
                ...
                if (parent == null) {
                    outputFile = outputFile.getFileSystem().getPath(filename);
                } else {
        ...
        return PartOutputStream.getFirstPart(outputFile);
    }
}
